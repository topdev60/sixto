    ...
    <div class="container-fluid">
        ...
        <div class="row gy-4">
            <div class="col-md-12 col-lg-8">
                <div class="card">
                    <div class="card-header">{{__('Riser/Casing/Liner-1/Liner-2/Hole')}}</div>
                        ...
                        <table id="dataTable" class="table table-responsive">
                            ...
                            <tbody>
                                    <tr @if($rigType == 0 || $rigType == 1 || $rigType == 2) style="display: none" @endif>
                                        <td class="text-center">{{__($wellbore->riser->RiserDescription)}}</td>
                                        <td class="text-center">{{__($wellbore->riser->RiserTop)}}</td>
                                        <td class="text-center">{{__($wellbore->riser->RiserBottom)}}</td>
                                        <td class="text-center">{{__($wellbore->riser->RiserID)}}</td>
                                        <td class="text-center">{{__($wellbore->riser->RiserOD)}}</td>
                                        <td class="text-center">{{__($wellbore->riser->RiserWeight)}}</td>
                                        <td class="text-center"><input type="checkbox" @if($wellbore->riser->RiserActive) checked @endif></td>
                                    </tr>
                                    <tr>
                                        <td class="text-center">{{__($wellbore->csg->CsgDescription)}}</td>
                                        <td class="text-center">{{__($wellbore->csg->CsgTop)}}</td>
                                        <td class="text-center">{{__($wellbore->csg->CsgBottom)}}</td>
                                        <td class="text-center">{{__($wellbore->csg->CsgID)}}</td>
                                        <td class="text-center">{{__($wellbore->csg->CsgOD)}}</td>
                                        <td class="text-center">{{__($wellbore->csg->CsgWeight)}}</td>
                                        <td class="text-center"> {{__(' ')}} </td>
                                    </tr>
                                    <tr>
                                        <td class="text-center">{{__($wellbore->lfirst->L1Description)}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center"><input type="checkbox" @if($wellbore->lfirst->L1Active) checked @endif></td>
                                    </tr>
                                    <tr>
                                        <td class="text-center">{{__($wellbore->lsecond->L2Description)}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center"><input type="checkbox" @if($wellbore->lsecond->L2Active) checked @endif></td>
                                    </tr>
                                    <tr>
                                        <td class="text-center">{{__($wellbore->hole->HoleDescription)}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__($wellbore->hole->HoleID)}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                        <td class="text-center">{{__(' ')}}</td>
                                    </tr>
                                                        ...
                                                        <td class="text-center">
                                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                                                    data-bs-target="#deleteNozzles{{$item->NZ_ID}}">
                                                                    <i class="fas fa-trash"></i>
                                                                </button>
        
                                                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                                                    data-bs-target="#editNozzles{{$item->NZ_ID}}">
                                                                    <i class="fas fa-pencil-alt"></i>
                                                                </button>
                                                            </div>
                                                            {{-- delete nozzle modal --}}
                                                            <div class="modal fade" id="deleteNozzles{{ $item->NZ_ID }}" data-bs-backdrop="static"
                                                                data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
                                                                aria-hidden="true">
                                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-body">
                                                                    <form action="{{ route('admin.nozzle.destroy') }}" method="post">
                                                                        @csrf
                                                                        <input type="hidden" name="nz_id" value="{{ $item->NZ_ID }}">
                                                            ...
                                                            {{-- delete nozzle modal --}}
                                                            {{-- edit nozzle modal --}}
                                                            <div class="modal fade" id="editNozzles{{ $item->NZ_ID }}" data-bs-backdrop="static"
                                                                data-bs-keyboard="false" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog modal-dialog-centered modal-lg">
                                                                <div class="modal-content">
                                                                    <form action="{{ route('admin.nozzle.update') }}" method="post">
                                                                    @csrf
                                                                    <input type="hidden" name="nz_id" value="{{ $item->NZ_ID }}">
                                                                    <div class="modal-header">
                                                                        <h5 class="modal-title" id="editModalLabel">Edit {{ __('Nozzle') }}</h5>
